...
开发环境和生产环境都不提供默认邀请码。需要在未提交的私有环境变量中配置学习邀请码与管理员邀请码；生产环境应先安全配置管理员身份，再由管理员逐个发放学员邀请码。`ADMIN_API_KEY` 只保留为内部应急访问方式，不能向测试用户发送。
