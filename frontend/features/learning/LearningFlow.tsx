...
function AiAnswerView({ result, onContinue }: { result: QuestionRequest; onContinue: () => Promise<void> }) {
  ...
  return <section className="flow-card ai-answer-card">
    ...
    {available ? <><p className="ai-answer-copy">{result.answer}</p><SpeakButton text={result.answer ?? ""} label="播报回答" /></> : <div className="ai-unavailable"><ShieldCheck aria-hidden="true" size={23} /><div><strong>课程还在等待专业审核</strong><p>{result.message}</p></div></div>}
...
function ConclusionView({ lesson, onContinue }: { lesson: Lesson; onContinue: () => void }) {
  return <section className="flow-card"><div className="content-status"><FileCheck2 aria-hidden="true" size={18} /><strong>内部测试候选内容</strong><span>待专业审核</span></div><p className="section-kicker">先记住这一句</p><h1 className="conclusion-title">{lesson.conclusion}</h1><div className="condition-box"><CircleHelp aria-hidden="true" size={21} /><div><strong>适用范围</strong><p>适用于普通家庭的家政入门知识学习，不等同于线下实操培训。</p></div></div><div className="safety-note"><ShieldCheck aria-hidden="true" size={21} /><div><strong>安全提醒</strong><p>{lesson.disclaimer}</p></div></div><SpeakButton text={`${lesson.conclusion}。安全提醒：${lesson.disclaimer}`} /><button className="rainbow-button" onClick={onContinue}><span>开始一步一步学</span><ArrowRight aria-hidden="true" size={22} /></button></section>;
}
